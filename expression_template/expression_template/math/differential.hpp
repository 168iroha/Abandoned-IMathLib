...
namespace iml {
	namespace op {
		namespace diff_op {
			...
			template <class T>
			struct is_zero_tag : std::false_type {};
			template <>
			struct is_zero_tag<zero_tag> : std::true_type {};
			template <class T>
			struct is_one_tag : std::false_type {};
			template <>
			struct is_one_tag<one_tag> : std::true_type {};
			...
			template <class Expr>
			struct Add<zero_tag, Expr, std::enable_if_t<!is_zero_tag<Expr>::value>> {
				static constexpr auto add(const zero_tag&, const Expr& rhs) -> Expr { return rhs; }
			};
			template <class Expr>
			struct Add<Expr, one_tag, std::enable_if_t<!is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Add<one_tag, Expr, std::enable_if_t<!is_one_tag<Expr>::value && !is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Sub<zero_tag, Expr, std::enable_if_t<!is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Sub<Expr, one_tag, std::enable_if_t<!is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Sub<one_tag, Expr, std::enable_if_t<!is_one_tag<Expr>::value && !is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Mul<zero_tag, Expr, std::enable_if_t<!is_zero_tag<Expr>::value>> {
				static constexpr auto mul(const zero_tag&, const Expr& rhs) -> zero_tag { return zero_tag(); }
			};
			template <class Expr>
			struct Mul<Expr, one_tag, std::enable_if_t<!is_zero_tag<Expr>::value>> {
				static constexpr auto mul(const Expr& lhs, const one_tag&) -> Expr { return lhs; }
			};
			template <class Expr>
			struct Mul<one_tag, Expr, std::enable_if_t<!is_one_tag<Expr>::value && !is_zero_tag<Expr>::value>> {
			...
			template <class Expr>
			struct Div<zero_tag, Expr, std::enable_if_t<!is_zero_tag<Expr>::value>> {
				static constexpr auto div(const zero_tag&, const Expr& rhs) -> zero_tag { return zero_tag; }
			};
			template <class Expr>
			struct Div<Expr, one_tag, std::enable_if_t<!is_zero_tag<Expr>::value>> {
				//呼び出されないはず
			};
			template <class Expr>
			struct Div<one_tag, Expr, std::enable_if_t<!is_one_tag<Expr>::value && !is_zero_tag<Expr>::value>> {
