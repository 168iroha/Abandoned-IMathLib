...
#include "IMathLib/media/image/image.hpp"
...
namespace iml {
	namespace ml {
		...
		struct FBO_OBJECT : ENABLER_OBJECT_BASE<GLuint, FBO_OBJECT> {
			// FBOオブジェクトのIDを保持
			using base_class = ENABLER_OBJECT_BASE<GLuint, FBO_OBJECT>;
			friend base_class;
		...
		private:
			// 初期状態のためのコンストラクタ
			FBO_OBJECT(enabler_object_init_tag) : base_class(), viewport_m() { base_class::init_state_construct(); }
			FBO_OBJECT(enabler_object_init_tag, const GLuint& fbo) : base_class(fbo), viewport_m() {
				base_class::init_state_construct();
			}
		public:
			// 通常のコンストラクタ
			FBO_OBJECT() : base_class() { base_class::construct(); screen_rect::id_m = 0; }
			FBO_OBJECT(const FBO_OBJECT& fbo) : base_class(fbo.id()), viewport_m(fbo.viewport_m) {
				base_class::construct();
			}
			FBO_OBJECT(FBO_OBJECT&& fbo) noexcept : base_class(std::move(fbo.obj_m)), viewport_m(fbo.viewport_m) {}
			FBO_OBJECT(const frame_buffer_object&);
			~FBO_OBJECT() {
				// 状態を持つならば処理する
				if (this->obj_m.get() != nullptr) base_class::destroy();
				
				// ここを抜けた時点でviewport_mのデストラクタが呼ばれて変更されたスクリーン上でglViewportが呼ばれる
			}

			// ENABLER_OBJECT_CONTROLのエイリアス
			using base_class::enabler_object_control_t;

			FBO_OBJECT& operator=(const FBO_OBJECT& v) {
				this->obj_m = v.obj_m;
				return *this;
			}
			FBO_OBJECT& operator=(FBO_OBJECT&& v) {
			...
			// IDの取得
			GLuint id() const { return *(this->obj_m); }
		...
		public:
			...
			void clear_color(const normal_color<float>& c = normal_color<float>()) {
				...
				glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, screen_rect::frame_buffer_object_id());
			}
			// デプスバッファとしてクリア
		...
		public:
			frame_buffer_object() : color_buffer_num_m(0), width_m(0), height_m(0) {
			...
			bool is_current() const {
				return (this->object_id_m != 0) && (this->object_id_m == screen_rect::frame_buffer_object_id());
			}
		...
		inline FBO_OBJECT::FBO_OBJECT(const frame_buffer_object& fbo) : base_class(fbo.id()), viewport_m(rect<int_t>(0, fbo.width(), fbo.height(), 0)) {
			base_class::construct();
		}
		inline void FBO_OBJECT::push_state() {
			glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, this->id());
			screen_rect::id_m = this->id();
			// フレームバッファが有効ならばスクリーン領域の変更
			if (this->id() != 0) {
				screen_rect::width_m = this->viewport_m.viewport_rect().right;
				screen_rect::height_m = this->viewport_m.viewport_rect().bottom;
			}
			// FBOを設定(スクリーンのターゲットの変更)してから初期化
			this->viewport_m.set_state();
			//this->viewport_m.unit_scaling();
		}
		...
		inline void set_frame_buffer_object(const frame_buffer_object& fbo) {
			using enabler_object_control_t = typename FBO_OBJECT::enabler_object_control_t;
			enabler_object_control_t::inst()->top()->set(fbo.id());
		}
#define IMATHLIB_SCREEN_FBO(NAME, FBO)	IMATHLIB_ENABLER_OBJECT(auto NAME = ::iml::ml::CREATE_FBO_OBJECT(FBO))
		...
		inline void glColor(const color_rgba& c) { ::glColor4ub(c.r, c.g, c.b, c.a); }
		inline void glColor(const color_rgb& c) { ::glColor3ub(c.r, c.g, c.b); }
		...
		inline void glColor(const normal_color<GLfloat>& color) { ::glColor4f(color.r, color.g, color.b, color.a); }
		inline void glColor(const normal_color<GLdouble>& color) { ::glColor4d(color.r, color.g, color.b, color.a); }
		...
		inline void draw_circle(const vector<T, N>& v, const T& r, size_t n = 30) {
			...
			for (size_t i = 0; i < n; ++i) {
				//x = r * cos(2 * pi_v<T> * ((double)i / n));
				//y = r * sin(2 * pi_v<T> * ((double)i / n));
				vertex.push_back(v[0] + r * cos(2 * pi_v<T> * ((double)i / n)));
				vertex.push_back(v[1] + r * sin(2 * pi_v<T> * ((double)i / n)));
			}
			glEnable(GL_POLYGON_SMOOTH);
			glHint(GL_POLYGON_SMOOTH_HINT, GL_DONT_CARE);
		...
		struct drawer_for_figure2 {
			...
			// 色
			drawer_for_figure2& color(const Float& r, const Float& g, const Float& b, const Float& a) { glColor(r, g, b, a); return *this; }
			drawer_for_figure2& color(const color_rgba& c) { glColor(c); return *this; }
			drawer_for_figure2& color(const Float& r, const Float& g, const Float& b) { glColor(r, g, b); return *this; }
			drawer_for_figure2& color(const color_rgb& c) { glColor(c); return *this; }
			drawer_for_figure2& color(const normal_color<Float>& c) { glColor(c); return *this; }
			// 大きさ
		...
		inline drawer_for_figure2<Float> rounded_rectangle(const vector2<Float>& v1, const vector2<Float>& v2, const Float r, size_t n = 10) {
			...
			// 右下の角
			for (; i <= n; ++i) {
				temp.push_back(v2[0] - r + r * cos(2 * pi_v<Float> * (Float(i) / sum_n)));
				temp.push_back(v2[1] - r + r * sin(2 * pi_v<Float> * (Float(i) / sum_n)));
			}
			// 左下の角
			for (; i <= 2 * n; ++i) {
				temp.push_back(v1[0] + r + r * cos(2 * pi_v<Float> * (Float(i) / sum_n)));
				temp.push_back(v2[1] - r + r * sin(2 * pi_v<Float> * (Float(i) / sum_n)));
			}
			// 左上の角
			for (; i <= 3 * n; ++i) {
				temp.push_back(v1[0] + r + r * cos(2 * pi_v<Float> * (Float(i) / sum_n)));
				temp.push_back(v1[1] + r + r * sin(2 * pi_v<Float> * (Float(i) / sum_n)));
			}
			// 右上の角
			for (; i <= sum_n; ++i) {
				temp.push_back(v2[0] - r + r * cos(2 * pi_v<Float> * Float(i) / sum_n));
		...
		inline drawer_for_figure2<Float> circle(const vector2<Float>& v, const Float& r, size_t n = 30) {
			...
			for (size_t i = 0; i <= n; ++i) {
				//x = r * cos(2 * pi_v<T> * ((double)i / n));
				//y = r * sin(2 * pi_v<T> * ((double)i / n));
				temp.push_back(v[0] + r * cos(2 * pi_v<Float> * (Float(i) / n)));
				temp.push_back(v[1] + r * sin(2 * pi_v<Float> * (Float(i) / n)));
			}
			return drawer_for_figure2<Float>(GL_POLYGON, std::move(temp));
		}
	}
